...
class Ping(threading.Thread):
    ...
    def run(self):
        ...
        while self.event.is_set():
            ...
            for node in consensus.validators:
...
class BOSNetHTTPServer(ThreadingMixIn, HTTPServer):
    ...
    def sequence_executor(self, func_name, arg):
...
class Transport(BaseTransport):
    ...
    def send(self, endpoint, data, retries=None):
        ...
        while n < retries:
            try:
                response = self.requests.post(urllib.parse.urljoin(endpoint.uri, '/scp_envelop'), data=post_data)
